...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        if self.att_model == 'transformer':
            ...
            self.cap_model = TransformerDecoder(self.rnn_size, 0, self.vocab_size,
                                                d_hidden=self.rnn_size // 2, n_layers=n_layers, n_heads=n_heads,
                                                drop_ratio=attn_drop)
    ...
    def forward(self, segs_feat, seq, gt_seq, num, ppls, gt_boxes, mask_boxes, ppls_feat, frm_mask, sample_idx,
                pnt_mask, opt, eval_opt={}):
        if opt == 'MLE':
            return self._forward(segs_feat, seq, gt_seq, ppls, gt_boxes, mask_boxes, num, ppls_feat, frm_mask,
                                 sample_idx, pnt_mask)
        elif opt == 'GRD':
            return self._forward(segs_feat, seq, gt_seq, ppls, gt_boxes, mask_boxes, num, ppls_feat, frm_mask,
                                 sample_idx, pnt_mask, True)
        elif opt == 'sample':
            seq, seqLogprobs, att2, sim_mat = self._sample(segs_feat, ppls, num, ppls_feat, sample_idx, pnt_mask,
                                                           eval_opt)
    ...
    def _forward(self, segs_feat, input_seq, gt_seq, ppls, gt_boxes, mask_boxes, num, ppls_feat, frm_mask, sample_idx,
                 pnt_mask, eval_obj_ground=False):
        ...
        if not self.enable_BUTD:
            ...
            pool_feats = torch.cat((F.layer_norm(pool_feats, [pool_feats.size(-1)]),
                                    F.layer_norm(loc_feats, [loc_feats.size(-1)]),
        ...
        if self.att_model == 'transformer':  # Masked Transformer does not support box supervision yet
            ...
            for i in range(self.seq_length):
                ...
                if not eval_obj_ground:
                    ...
                    output, state, att2_weight, att_h, max_grd_val, grd_val = self.core(xt, fc_feats,
                                                                                        conv_feats, p_conv_feats,
                                                                                        pool_feats, p_pool_feats,
                                                                                        pnt_mask, frm_mask_on_prop,
            ...
            if not eval_obj_ground:
                ...
                lm_loss, att2_loss, ground_loss = self.critLM(decoded, att2_weights, ground_weights,
                                                              seq[:, 1:seq_cnt + 1].clone(),
                                                              roi_labels[:, :seq_cnt, :].clone(),
                                                              input_seq[:, 1:seq_cnt + 1, 0].clone())
                ...
                return cls_pred, \
                       torch.max(att2_weights.view(seq_batch_size, seq_cnt, self.num_sampled_frm,
                                                             self.num_prop_per_frm), dim=-1)[1], \
                       torch.max(ground_weights.view(seq_batch_size,
            ...
            for t in range(self.seq_length + 1):
                ...
                if t < self.seq_length:
                    rnn_output, state, att2_weight, att_h, _, _ = self.core(xt, fc_feats, conv_feats,
                                                                            p_conv_feats, pool_feats, p_pool_feats,
                                                                            att_mask, pnt_mask, state,
    ...
    def _sample_beam(self, segs_feat, ppls, num, ppls_feat, sample_idx, pnt_mask, opt={}):
        ...
        if not self.enable_BUTD:
            ...
            pool_feats = torch.cat(
                (F.layer_norm(pool_feats, [pool_feats.size(-1)]), F.layer_norm(loc_feats, [loc_feats.size(-1)]),
                 F.layer_norm(label_feat, [label_feat.size(-1)])), 2)
        ...
        for k in range(batch_size):
            ...
            beam_sim_mat_static_update = sim_mat_static_update[k:k + 1].expand(beam_size, self.detect_size + 1,
                                                                               rois_num)

            rnn_output, state, att2_weight, att_h, _, _ = self.core(xt, beam_fc_feats, beam_conv_feats,
                                                                    beam_p_conv_feats, beam_pool_feats,
                                                                    beam_p_pool_feats, beam_pnt_mask, beam_pnt_mask,
            ...
            self.done_beams[k] = self.beam_search(state, rnn_output, beam_fc_feats, beam_conv_feats, beam_p_conv_feats,
                                                  beam_pool_feats, beam_p_pool_feats, beam_sim_mat_static_update,
                                                  beam_ppls, beam_pnt_mask, vis_offset, roi_offset, opt)
