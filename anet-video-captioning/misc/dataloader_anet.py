...
class DataLoader(data.Dataset):
    def __init__(self, opt, split='training', seq_per_img=5, num_proposals=None, label_proposals=None):
        self.opt = opt
        ...
        if num_proposals is None or label_proposals is None:
            ...
            h5_proposal_file = h5py.File(
                self.opt.proposal_h5, 'r', driver='core')
            ...
            self.label_proposals = label_proposals
    ...
    def __getitem__(self, index):
                    ...
                    if caption['idx'][i][j] < self.seq_length:
                        if self.test_mode:
                            ...
                            bbox_ann.append({'bbox': caption['bbox'][i], 'label': self.dtoi[cls], 'clss': cls,
                                             'bbox_idx': bbox_idx, 'idx': caption['idx'][i][j],
        ...
        #         try:
        #             img = Image.open(os.path.join(self.opt.image_path, seg_id, str(i + 1).zfill(2) + '.jpg')).convert(
        #                 'RGB')
        ...
        pad_pnt_mask = torch.from_numpy(pad_pnt_mask).bool()
        pad_frm_mask = torch.from_numpy(pad_frm_mask).bool()
        pad_box_mask = torch.from_numpy(pad_box_mask).bool()

        pad_proposals.masked_fill_(pad_pnt_mask.view(-1, 1), 0.)
        pad_region_feature.masked_fill_(pad_pnt_mask.view(-1, 1), 0.)

        num = torch.FloatTensor([ncap, num_pps, num_box, int(seg_id_ix), max(self.num_seg_per_vid[vid_id_ix]) + 1,
                                 timestamps[0] * 1. / dur, timestamps[1] * 1. / dur])  # 3 + 4 (seg_id, num_of_seg_in_video, seg_start_time, seg_end_time)
        sample_idx = torch.from_numpy(sample_idx).long()

        # if self.vis_attn:
        #     return seg_feature, input_seq, gt_seq, num, pad_proposals, pad_gt_bboxs, pad_box_mask, seg_id, seg_show, \
        #            seg_dim_info, pad_region_feature, pad_frm_mask, sample_idx, pad_pnt_mask
        # else:
        return seg_feature, input_seq, gt_seq, num, pad_proposals, pad_gt_bboxs, pad_box_mask, seg_id, \
            pad_region_feature, pad_frm_mask, sample_idx, pad_pnt_mask
